...
export default {
  ...
  methods: {
    ...
    async saveForm() {
      let { title, description, quizzes, backGroundImg, bottomButton } = this.editingForm
      this.isLoading = true
      await this.orgUpdateForm({
        formId: this.formId,
        formDetail: { title, description, quizzes, backGroundImg, bottomButton },
      })
      this.isLoading = false
    ...
    async publishForm() {
      ...
      await this.orgUpdateForm({
        formId: this.formId,
        formDetail: { title, description, quizzes, state: 1, backGroundImg, bottomButton },
      })
      this.isLoading = false
