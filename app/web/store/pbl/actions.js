...
const actions = {
  ...
  async createNewProject(context, { description, name, privilege, type, visibility, siteId, tags }) {
    let projectDetail = await keepwork.projects.createProject({
      description,
      name,
      privilege,
      type,
  ...
  async getTypeProjects({ commit }, payload) {
    try {
      const projects = await EsAPI.projects.getProjects(payload)
  ...
  async getMyAllProjects({ commit }) {
    await Promise.all([keepwork.projects.getPersonalProjects(), keepwork.projects.getContributeProjects()])
      .then(res => {
  ...
  async getProjectApplyList(context, { objectId, objectType, applyType }) {
    let { commit } = context
    await keepwork.applies
      .getApplyList({ objectId, objectType, applyType })
      .then(projectApplyList => {
  ...
  async getProjectMember(context, { objectId, objectType }) {
    let { commit } = context
    await keepwork.members
      .getProjectMembersList({ objectId, objectType })
      .then(memberList => {
  ...
  async applyJoinProject(context, { objectType = 5, objectId, applyType, applyId, legend, extra }) {
    let { dispatch } = context
    await keepwork.applies
      .applyProjectMember({ objectType, objectId, applyType, applyId, legend, extra })
      .then(async () => {
  ...
  async getComments(context, { objectType = 5, objectId, xPage, xPerPage, xOrder }) {
    let { commit } = context
    let commentList = await keepwork.comments
      .getComments({ objectType, objectId, xPage, xPerPage, xOrder })
      .catch(error => {
        return Promise.reject(error)
      })
  ...
  async createComment(context, { objectType = 5, objectId, content }) {
    let { dispatch } = context
    let newComment = await keepwork.comments
      .createComment({ objectType, objectId, content })
      .catch(error => {
        return Promise.reject(error)
      })
  ...
  async getProjectIssues({ commit }, payload) {
    await keepwork.issues
      .getSingleProjectIssues(payload)
      .then(projectIssues => {
  ...
  async getUserFavorite({ commit }, { objectType, userId }) {
    await keepwork.favorites
      .getUserFavorites({ objectType, userId })
      .then(res => {
