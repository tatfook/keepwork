...
export default {
  ...
  methods: {
    ...
    //继续学习
    goStudy() {
      var link = this.nextLearnLesson
        ? this.nextLearnLesson
        : this.firstLessonUrl
    ...
    async UserState() {
      ...
      if (!this.editMode) {
        ...
        if (r.err == 0) {
          ...
          //学习进度以及学习情况
          var proNum = Number(
            (r.data.doneCount / r.data.lessonCount * 100).toFixed(1)
          )
          ...
          for (var i = 0; i < this.properties.data.lessons.length; i++) {
            ...
            for (var j = 0; j < r.data.lessons.length; j++) {
              if (
                this.properties.data.lessons[i].lessonUrl ==
                r.data.lessons[j].lessonUrl
              ) {
                this.properties.data.lessons[i].learnedFlag =
                  r.data.lessons[j].learnedFlag
              }
            }
          }
        } else if (r.err == 102) {
...
.lesson-item-desc .lesson-item-goals,
