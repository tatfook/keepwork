...
let MxUtils = {
  ...
  parseXml: (function() {
    if (window.DOMParser) {
      ...
      // IE<=9
      return function(xml) {
        let result = MxUtils.createXmlDocument()
        result.async = false
  ...
  getPortConstraints: function(terminal, edge, source, defaultValue) {
    ...
    if (value === null) {
      ...
      let constraintRotationEnabled = MxUtils.getValue(
        terminal.style,
  ...
  intersection: function(x0, y0, x1, y1, x2, y2, x3, y3) {
    ...
    let numeA = (x3 - x2) * (y0 - y2) - (y3 - y2) * (x0 - x2)
    let numeB = (x1 - x0) * (y0 - y2) - (y1 - y0) * (x0 - x2)
   ...
   * MxUtils.setCellStyleFlags(graph.model, cells, MxConstants.STYLE_FONTSTYLE, MxConstants.FONT_BOLD);
   * (end)
   *
  ...
  printScreen: function(graph) {
    let wnd = window.open()
    let bounds = graph.getGraphBounds() // eslint-disable-line no-unused-vars
