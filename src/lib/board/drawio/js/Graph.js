...
export default class Graph {
  constructor(container, model, renderHint, stylesheet, themes) {
    ...
    if (this.edgeMode) {
      ...
      let mouseDown = null // eslint-disable-line no-unused-vars
          ...
          if (this.isEnabled() && !this.panningHandler.isActive() && !MxEvent.isControlDown(me.getEvent()) && !MxEvent.isShiftDown(me.getEvent()) && !MxEvent.isAltDown(me.getEvent())) {
            ...
            if (start.point !== null && start.state !== null && start.event !== null) {
              ...
              if (Math.abs(start.point.x - me.getGraphX()) > tol || Math.abs(start.point.y - me.getGraphY()) > tol) {
                ...
                if (
                  handler !== null && handler.bends !== null && handler.bends.length > 0) {
                        ...
                        if (pts !== null && ((edgeStyle === null && handle === null) || edgeStyle === MxEdgeStyle.OrthConnector)) {
                          ...
                          if (handle === null) {
                            ...
                            if (MxUtils.contains(box, pts[0].x, pts[0].y)) {
                              ...
                              let nobends = edgeStyle !== null && (pts.length === 2 || (pts.length === 3 && ((Math.round(pts[0].x - pts[1].x) === 0 && Math.round(pts[1].x - pts[2].x) === 0) || (Math.round(pts[0].y - pts[1].y) === 0 && Math.round(pts[1].y - pts[2].y) === 0))))
    ...
    if (typeof MxVertexHandler !== 'undefined') {
      ...
      let connectionHandleIsOutlineConnectEvent = this.connectionHandler
        .isOutlineConnectEvent

      this.connectionHandler.isOutlineConnectEvent = function(me) {
        return (
          (this.currentState !== null && me.getState() === this.currentState && timeOnTarget > 2000) || ((this.currentState === null || MxUtils.getValue(this.currentState.style, 'outlineConnect', '1') !== '0') && connectionHandleIsOutlineConnectEvent.apply(this, arguments)))
      }
      ...
      rubberband.isForceRubberbandEvent = function(me) {
        return (isForceRubberBandEvent.apply(this, arguments) || (MxUtils.hasScrollbars(this.graph.container) && MxClient.IS_FF && MxClient.IS_WIN && me.getState() === null && MxEvent.isTouchEvent(me.getEvent())))
      }
        ...
        MxUtils.bind(this, function(sender, evt) {
          if (!MxEvent.isMultiTouchEvent(evt)) {
            ...
            if (cell === null) {
              ...
              this.getSelectionCount() > 1 && this.isCellSelected(cell)
            ) {
      ...
      this.selectionModel.addListener(
        ...
        MxUtils.bind(this, function() {
          ...
          if (
            ch.currentFocus !== null && ch.isStateIgnored(ch.currentFocus, true)
          ) {
 ...
 */
Graph.prototype.baseUrl = (window !== window.top
  ? document.referrer
...
Graph.prototype.labelLinkClicked = function(state, elt, evt) {
  ...
  if (href !== null && !this.isPageLink(href)) {
    if (!this.isEnabled()) {
      let target = state.view.graph.isBlankLink(href)
...
Graph.prototype.initLayoutManager = function() {
  ...
  this.layoutManager.getLayout = function(cell) {
    ...
    if (style['childLayout'] === 'stackLayout') {
      ...
      let flowLayout = new MxHierarchicalLayout(
        this.graph,
...
Graph.prototype.updatePlaceholders = function() {
  let model = this.model // eslint-disable-line no-unused-vars
  let validate = false
...
Graph.prototype.formatDate = function(date, mask, utc) {
  ...
  let token1 = /d{1,4}|m{1,4}|yy(?:yy)?|([HhMsTt])\1?|[LloSZ]|"[^"]*"|'[^']*'/g
  ...
  let token = [ token1, timezone, timezoneClip, pad ]
...
Graph.prototype.createLayersDialog = function() {
  ...
  for (let i = 0; i < childCount; i++) {
    ;(function(layer) {
      ...
      span.appendChild(cb)
      let title = layer.value || (MxResources.get('background') || 'Background')
...
Graph.prototype.connectVertex = function(
  ...
  try {
    ...
    if (realTarget === null && duplicate) {
      ...
      geo = this.getCellGeometry(realTarget)
    ...
    if (edge !== null && this.connectionHandler.insertBeforeSource) {
      let index = null // eslint-disable-line no-unused-vars
      let tmp = source
...
HoverIcons.prototype.triangleUp = !MxClient.IS_SVG
  ...
  : Graph.createSvgImage(
    26,
    14,
    '<path d="m 1 14 L 13 1 L 26 14 z" stroke="#fff" fill="#007dfc"/>'
  )
...
HoverIcons.prototype.triangleRight = !MxClient.IS_SVG
  ...
  : Graph.createSvgImage(
    14,
    26,
    '<path d="m 1 1 L 14 13 L 1 26 z" stroke="#fff" fill="#007dfc"/>'
  )
...
HoverIcons.prototype.triangleDown = !MxClient.IS_SVG
  ...
  : Graph.createSvgImage(
    26,
    14,
    '<path d="m 1 1 L 13 14 L 26 1 z" stroke="#fff" fill="#007dfc"/>'
  )
...
HoverIcons.prototype.triangleLeft = !MxClient.IS_SVG
  ...
  : Graph.createSvgImage(
    14,
    26,
    '<path d="m 14 1 L 1 13 L 14 26 z" stroke="#fff" fill="#007dfc"/>'
  )
...
HoverIcons.prototype.roundDrop = !MxClient.IS_SVG
  ...
  : Graph.createSvgImage(
    26,
    26,
    '<circle cx="13" cy="13" r="12" stroke="#fff" fill="#007dfc"/>'
  )
...
HoverIcons.prototype.click = function(state, dir, me) {
  ...
  if (
    ...
    let cells = this.graph.connectVertex(
      state.cell,
      dir,
...
;(function() {
  ...
  MxConnector.prototype.paintLine = function(c, absPts, rounded) {
    ...
    if (
      ...
      let f = Editor.jumpSizeRatio // eslint-disable-line no-unused-vars
      let moveTo = true
      let last = null
      let len = null
      let pts = []
...
MxStencilRegistry.loadStencil = function(filename, fn) {
  if (fn !== null) {
    let req = MxUtils.get(// eslint-disable-line no-unused-vars
      filename,
...
if (typeof MxVertexHandler !== 'undefined') {
  ;(function() {
    ...
    let mxConnectionHandlerCreateTarget =
    ...
    Graph.prototype.importGraphModel = function(node, dx, dy, crop) {
      ...
      let targetChildCount = this.model.getChildCount(this.model.getRoot()) // eslint-disable-line no-unused-vars
    ...
    Graph.prototype.getPreferredSizeForCell = function(cell) {
      let result = MxGraph.prototype.getPreferredSizeForCell.apply(
        this,
        arguments
      )
    ...
    Graph.prototype.replaceElement = function(elt, tagName) {
      let span = elt.ownerDocument.createElement(
    ...
    Graph.prototype.getDropTarget = function(cells, evt, cell, clone) {
      let model = this.getModel() // eslint-disable-line no-unused-vars
          ...
          if (
            ...
            if (this.currentLink !== null) {
              ...
              if (!MxEvent.isConsumed(evt)) {
                let target = blank ? graph.linkTarget : '_top'
                ...
                if (target === '_self' && window !== window.top) {
                  ...
                  // Avoids page reload for anchors (workaround for IE but used everywhere)
                  if (
                    this.currentLink.substring(0, graph.baseUrl.length) ===
                      graph.baseUrl &&
    ...
    Graph.prototype.duplicateCells = function(cells, append) {
      ...
      try {
        ...
        for (let i = 0; i < cells.length; i++) {
          ...
          let child = this.moveCells([clones[i]], s, s, false, parent)[0]
          select.push(child)
    ...
    Graph.prototype.getSvg = function(
      ...
      root.setAttribute(
        'width',
        Math.max(1, Math.ceil(bounds.width * s) + 2 * border) + 'px'
      )
      root.setAttribute(
        'height',
        Math.max(1, Math.ceil(bounds.height * s) + 2 * border) + 'px'
      )
    ...
    Graph.prototype.insertRow = function(table, index) {
      ...
      let cols = bd.rows.length > 0 ? bd.rows[0].cells.length : 1
    ...
    Graph.prototype.pasteHtmlAtCaret = function(html) {
      ...
      if (window.getSelection) {
        ...
        if (sel.getRangeAt && sel.rangeCount) {
          ...
          let node
          var lastNode // eslint-disable-line no-unused-vars
    ...
    HoverIcons.prototype.mainHandle = !MxClient.IS_SVG
      ...
      : Graph.createSvgImage(
        18,
        18,
        '<circle cx="9" cy="9" r="5" stroke="#fff" fill="#007dfc" stroke-width="1"/>'
      )
    ...
    HoverIcons.prototype.fixedHandle = !MxClient.IS_SVG
      ...
      : Graph.createSvgImage(
        18,
        18,
        '<circle cx="9" cy="9" r="5" stroke="#fff" fill="#007dfc" stroke-width="1"/><path d="m 7 7 L 11 11 M 7 11 L 11 7" stroke="#fff"/>'
      )
    HoverIcons.prototype.terminalHandle = !MxClient.IS_SVG
      ...
      : Graph.createSvgImage(
        18,
        18,
        '<circle cx="9" cy="9" r="5" stroke="#fff" fill="#007dfc" stroke-width="1"/><circle cx="9" cy="9" r="2" stroke="#fff" fill="transparent"/>'
      )
    ...
    if (MxClient.IS_SVG) {
      MxConstraintHandler.prototype.pointImage = Graph.createSvgImage(
        5,
        5,
        '<path d="m 0 0 L 5 5 M 0 5 L 5 0" stroke="#007dfc"/>'
      )
    }
    ...
    MxRubberband.prototype.mouseMove = function(sender, me) { // 函数小括号内的数
    ...
    console.log(mxEdgeHandlerUpdatePreviewState)

    MxEdgeHandler.prototype.updatePreviewState = function(
      // edge, // eslint-disable-line no-unused-vars
      // point, // eslint-disable-line no-unused-vars
      terminalState // eslint-disable-line no-unused-vars
      // me // eslint-disable-line no-unused-vars
    ) {
      // console.log(point)
    ...
    MxEdgeHandler.prototype.createHandleShape = function(index) { // virtual小括号内数值
    ...
    MxVertexHandler.prototype.createSizerShape = function(index) { // bounds,fillColor括号内数值
      this.handleImage =
    ...
    let mxVertexHandlerMouseDown = MxVertexHandler.prototype
    mxVertexHandlerMouseDown.mouseDown = (me) => { // eslint-disable-line no-unused-vars
      // sender,me  小括号内数值
      let model = this.graph.getModel()
      ...
      if (
        ...
        mxVertexHandlerMouseDown.mouseDown.apply(this, arguments)
      }
    }
