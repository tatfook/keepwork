...
import MxKeyHandler from '@/lib/board/mxGraph/handler/MxKeyHandler'
import MxEventObject from '@/lib/board/mxGraph/util/MxEventObject'
import Menus from './Menus'
import MxClipboard from '@/lib/board/mxGraph/util/MxClipboard'
import MxPoint from '@/lib/board/mxGraph/util/MxPoint'
import MxRectangle from '@/lib/board/mxGraph/util/MxRectangle'
import { HoverIcons, Graph } from './Graph'
import MxObjectCodec from '@/lib/board/mxGraph/io/MxObjectCodec'
import MxCodecRegistry from '@/lib/board/mxGraph/io/MxCodecRegistry'
import Toolbar from './Toolbar'
import Sidebar from './Sidebar'
import Format from './Format'
import Dialog from '@/lib/board/Dialog'
import MxXmlRequest from '@/lib/board/mxGraph/util/MxXmlRequest'
import MxMorphing from '@/lib/board/mxGraph/util/MxMorphing'
import MxImage from '@/lib/board/mxGraph/util/MxImage'
import MxOutline from '@/lib/board/mxGraph/view/MxOutline'
import MxStackLayout from '@/lib/board/mxGraph/layout/MxStackLayout'
import {
  ColorDialog,
  OpenDialog,
  FilenameDialog,
      ...
      MxUtils.bind(this, (sender, evt) => {
        ...
        if (this.toolbar !== null) {
          ...
          if (this.toolbar.edgeStyleMenu !== null) {
            ...
            let edgeStyleDiv = this.toolbar.edgeStyleMenu.getElementsByTagName(
              'div'
            )[0]
          ...
          if (this.toolbar.edgeShapeMenu !== null) {
            ...
            let edgeShapeDiv = this.toolbar.edgeShapeMenu.getElementsByTagName(
              'div'
            )[0]
          ...
          if (this.toolbar.lineStartMenu !== null) {
            let lineStartDiv = this.toolbar.lineStartMenu.getElementsByTagName(
              'div'
            )[0]
          ...
          if (this.toolbar.lineEndMenu !== null) {
            let lineEndDiv = this.toolbar.lineEndMenu.getElementsByTagName(
              'div'
            )[0]
...
EditorUi.prototype.initCanvas = () => {
  ...
  if (this.editor.chromeless) {
    ...
    if (window.urlParams['toolbar'] !== '0') {
      ...
      if (
        graph.lightbox &&
        (window.urlParams['close'] === '1' || this.container !== document.body)
      ) {
        addButton(
...
EditorUi.prototype.refresh = sizeDidChange => {
  ...
  let effHsplitPosition = Math.max(
    0,
